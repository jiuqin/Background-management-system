<template>
  <div>
    ...
    >
      <el-table-column prop="id" label="编号" sortable width="180" style=" text-align: center;">
      </el-table-column>
      <el-table-column prop="title" label="轮播图标题"  width="180">
      </el-table-column>
      <el-table-column prop="img" label="图片">
        <template slot-scope="item">
             <img :src="$api.baseUrl+item.row.img" style="width:120px;height:100px">
        </template>
      </el-table-column>
        ...
        .then(() => {
          this.$axios.post(this.$api. bannerdeleteUrl, { id }).then((res) => {
            if (res.data.code == 200) {
              this.bannerlist = res.data.list;//更新页面上的数据
              this.$message({
                type: "success",
                message: "删除成功!",
              });
